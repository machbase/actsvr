...
type MQTTConfig struct {
	...
	MaxReconnectInterval time.Duration `json:"maxReconnectInterval"`
}
...
func NewK2MBroker(config *K2MConfig, logger *util.Log) (*K2MBroker, error) {
	...
	if len(routes) == 0 && len(config.TopicMappings) > 0 {
		// Convert legacy TopicMappings to Routes for backward compatibility
		routes = make([]RouteConfig, len(config.TopicMappings))
...
func (b *K2MBroker) initMQTTClient() error {
	...
	opts.SetKeepAlive(b.config.MQTTConfig.KeepAlive)
	opts.SetPingTimeout(b.config.MQTTConfig.PingTimeout)
	opts.SetConnectRetry(b.config.MQTTConfig.ConnectRetry)
	opts.SetMaxReconnectInterval(b.config.MQTTConfig.MaxReconnectInterval)
...
func (b *K2MBroker) initKafkaConsumer() error {
	...
	config.Consumer.Group.Session.Timeout = b.config.KafkaConfig.SessionTimeout
	config.Consumer.Group.Heartbeat.Interval = b.config.KafkaConfig.HeartbeatInterval
